...
from src.constants import config, EMBEDDING_MODEL, SIMILARITY_THRESHOLD
...
# @lru_cache
def get_similarity(
    query: str, threshold: float = SIMILARITY_THRESHOLD
) -> Optional[SimilarityResponse]:
    store = get_vector_store()
    response = store.similarity_search_with_relevance_scores(
        query, score_threshold=threshold, k=1
    )
